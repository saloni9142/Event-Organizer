...
export default function AIEventCreator({ onEventGenerated }) {
  ...
  useEffect(() => {
    if (isOpen) {
      // Focus the textarea when the dialog opens (next tick to ensure mount)
      ...
      <DialogContent>
        ...
        <div className="space-y-4">
          <Textarea
            ...
            value={prompt}
            onChange={(e) => {
              // Debugging logs help detect whether keyboard events reach the textarea
              if (process.env.NODE_ENV !== "production") console.debug("textarea onChange:", e.target.value);
              setPrompt(e.target.value);
            }}
            onInput={(e) => {
              if (process.env.NODE_ENV !== "production") console.debug("textarea onInput:", e.target.value);
            }}
            onKeyDown={(e) => {
              if (process.env.NODE_ENV !== "production") console.debug("textarea keydown:", e.key);
            }}
